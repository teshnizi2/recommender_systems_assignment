...
- **The codebook collapsed.** We applied the standard mitigations (EMA updates, small uniform init, dead-code resets, denoising warmup) but the RQ-VAE still narrowed to 1 / 20 / 16 live entries at levels 0 / 1 / 2. Sentence-T5 outputs on this catalogue are strongly anisotropic; under the 50 GPU-hour ablation budget we did not pursue more aggressive mitigations (k-means init with cosine-distance quantisation, contrastive pretraining of the content encoder). See `report/report.pdf` §4.2 for the full diagnostic.
