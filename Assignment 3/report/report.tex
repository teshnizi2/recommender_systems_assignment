...
\begin{table}[h]
  ...
  \caption{Default-config test metrics on Amazon Toys and Games
    ($N{=}19{,}412$ users, beam~$20$).}
  \label{tab:default}
  \begin{tabular}{lcccc}
    \toprule
    & Recall@5 & NDCG@5 & Recall@10 & NDCG@10 \\
    \midrule
    TIGER (ours)    & 0.0342 & 0.0245 & 0.0488 & 0.0292 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Recall@5 falls slightly below the paper neighbourhood, NDCG@5 is on
the lower end of the expected range, and Recall@10/NDCG@10 are within
~10\% of the paper's published numbers for Toys and Games.
...
\begin{figure}[h]
  ...
  \caption{Training curves for the default configuration.  Left:
    RQ-VAE reconstruction + commitment losses converge within ~50
    epochs.  Right: Transformer training and validation NDCG@10 over
    the seq2seq schedule.}
  \label{fig:training}
\end{figure}
...
initialisation, and assigns $11{,}924$ items to $\sim\!11{,}000$
unique $L$-tuples (Figure~\ref{fig:codebook} and
Figure~\ref{fig:collision}).  The remaining items collide and are
disambiguated by the suffix token.
...
A vanilla VQ-VAE on these Sentence-T5 inputs collapses to fewer than
$60$ unique Semantic IDs across all $11{,}924$ items
(Section~\ref{sec:discussion} unpacks why).  Combining
(i)~input centring against the global Sentence-T5 mean direction,
(ii)~denoising warmup, (iii)~$k$-means$\,$++ initialisation of every
codebook level on warmed-up encoder outputs, and (iv)~EMA codebook
updates with periodic dead-code resetting is what made the codebook
actually usable.
...
This suggests $30$ epochs is not enough to train a deeper model
through its larger optimisation surface.
...
metric by only a few absolute percent.  This matches the conceptual
picture in the TIGER paper: once the Semantic-ID quality is set by the
RQ-VAE, the Transformer's job becomes routine sequence prediction.
If the Semantic IDs are bad, no Transformer width fixes them.
...
significant.
(ii)~A learning-rate decay schedule.  Even on the default config
training loss was still falling at the early-stop point; a cosine
schedule with a longer warmup might push Recall@5 into the upper
$0.04$s reported by the paper.
...
achieves \textbf{Recall@10 = 0.0488} and \textbf{NDCG@10 = 0.0292} on
the held-out test split with beam size~$20$.  Ablating $K$, $L$,
Transformer architecture, and beam size shows that Semantic-ID
quality (controlled by $K$ and $L$) is the dominant axis: a single
codebook-size or level mistake costs an order of magnitude in
Recall@5, while doubling Transformer depth or width changes the
same metric by only a few absolute percent.  The two practical
lessons we want to flag are (a) that getting the RQ-VAE to use its
codebook is the single hardest part of a TIGER reimplementation --
vanilla VQ-VAE losses collapse on Sentence-T5 outputs and need
centring, denoising warmup, $k$-means$\,$++ initialisation, EMA
updates, and dead-code resetting to recover -- and (b) that the
resulting Semantic-ID prefixes \emph{are} interpretable: items
sharing the first-level token are semantically coherent and the
second-level token narrows to genre-style sub-clusters.
